...
export function DesignPreview() {
  ...
  return (
    <main className="sn-shell sn-design-board">
      ...
      {activePage !== 'coach' && <section className="sn-subpage"><span className="sn-eyebrow">{activePage === 'practice' ? 'Practice queue' : activePage === 'progress' ? 'Learning profile' : 'Preferences'}</span><h1>{activePageLabels[activePage]}</h1><p>{activePage === 'practice' ? 'Pick a challenge and let the coach meet you where you are.' : activePage === 'progress' ? 'Your patterns become clearer one thoughtful attempt at a time.' : 'Your name and microphone permission live on this device for now.'}</p></section>}
      {activePage !== 'coach' ? null : <>
      ...
      <section className="sn-coach sn-coach--preview"><div className="sn-coach-meta"><span><i /> AI COACH · GUIDANCE</span><span className="sn-level">Hint 1 of 4</span></div><p>When you are standing at the number <b>2</b> and looking for its partner, what exactly are you searching the rest of the array for?</p><p className="sn-analogy">A good question is a small door. Let&apos;s find the handle.</p></section>
      ...
      <ExplanationInput question={null} disabled={false} onSubmit={() => undefined} />
...
const activePageLabels: Record<PanelPage, string> = { coach: 'Coach room', practice: 'Practice queue', progress: 'Your progress', settings: 'Settings' };